...
class GoalDropper:
    ...
    def find_goal_block(self, agent_id):
        ...
        for found_goal_block in self.agent.found_goal_blocks:
            if found_goal_block["colour"] == current_drop_off["colour"] and found_goal_block["shape"] == \
                    current_drop_off["shape"]:
        ...
        if len(distances) > 0:
            ...
            return np.array(current_goal_blocks_found)[idx][0], np.array(distances)[idx][0]
    ...
    def grab_conflict(self, goal_block, distance):
        ...
        for other_agent in self.agent.other_agents:
            if other_agent["phase"] == "CHOOSE_GOAL" and other_agent["location"] is not None:
                other_goal_block, other_distance = self.find_goal_block(other_agent["agent_id"])
